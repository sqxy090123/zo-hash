"""
DY哈希模块
包含原 dy1（保留原逻辑）和新版 dy64/dy128/dy256/dy512（采用流式随机采样）
"""
...
def _dy_generic(path: str, output_bits: int, max_steps: int = 256, max_len: int = 64) -> str:
    ...
    # 结果变量，保持 output_bits 位
    result = 0

    # 掩码，用于保留低 output_bits 位
        ...
        for _ in range(max_steps):
            ...
            for i in range(length):
                ...
                if not b:
                    # 理论上不会发生，因为文件非空
                    continue
                seed_bytes.append(b[0])
                # 合并每个字节到结果
                transformed = (b[0] * 0x9E3779B97F4A7C15) ^ 0x1234567890ABCDEF
                transformed &= 0xFFFFFFFFFFFFFFFF
                result ^= transformed
